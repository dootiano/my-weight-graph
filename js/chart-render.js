...
export function renderChart(records, userProfile, canvasMain, canvasBar, options = {}) {
  ...
  const wkAvg=new Map();
  byWeek.forEach((wp,wk)=>{ if(wp.length<4)return; wkAvg.set(wk,{avg:+(wp.reduce((s,p)=>s+p.w,0)/wp.length).toFixed(2),label:wkLabel(wk),x:Math.max(...wp.map(p=>p.t))}); });
  ...
  function drawBox(ctx,dotX,dotY,bx,by,lines,col,chart,bp=BP,blh=BLH,f0=10,f1=9){
    ...
    lines.forEach((l,i)=>{ ctx.fillStyle='#fff'; ctx.font=i===0?`bold ${f0}px sans-serif`:`${f1}px sans-serif`; ctx.textAlign='left'; ctx.fillText(l,bx+bp,by+bp+blh*i+blh*0.67); });
    ctx.restore();
  }
  ...
  if(show7dayMA) datasets.push({label:'7일 이동평균',data:ma7,borderColor:ORANGE,backgroundColor:'transparent',borderWidth:1.8,borderDash:[7,4],pointRadius:0,tension:.3,order:2});
  ...
  canvasMain._chartInstance = new Chart(canvasMain.getContext('2d'),{
    ...
    options:{
      ...
      scales:{
        ...
        y:{min:yMin,max:yMax,grid:{color:GRID},ticks:{color:TICK,font:{size:11},stepSize:5,callback:v=>v},border:{color:'rgba(255,255,255,.15)'},title:{display:true,text:'체중 (kg)',color:TICK,font:{size:11}},afterFit(s){s.width=Y_AXIS_W;}},
        y2:{min:yMin,max:yMax,position:'right',grid:{drawOnChartArea:false},ticks:{color:TICK,font:{size:11},stepSize:5,callback:v=>v},border:{color:'rgba(255,255,255,.15)'},afterFit(s){s.width=isMobile?0:Y_AXIS_W;}}
      },
...
export function calcStats(records, userProfile) {
  const goal=userProfile?.goal??80, height=userProfile?.height??null, birthYear=userProfile?.birthYear??null;
  ...
  const changes=[]; for(let i=1;i<vk.length;i++) changes.push(avgs.get(vk[i])-avgs.get(vk[i-1]));
  ...
  if(r6.length>=2&&curW>goal){
    const rl=r6[0].weight-r6[r6.length-1].weight, rd=(new Date(r6[r6.length-1].date)-new Date(r6[0].date))/86400000, rate=rd>0?rl/rd:0;
    if(rate>0){const d=new Date(end.getTime()+Math.round((curW-goal)/rate)*86400000); eta=`${d.getFullYear()}년 ${d.getMonth()+1}월 ${d.getDate()}일`;}
  }
  return {startW:maxW,curW,minW,goal,days,loss:+loss.toFixed(1),daily:+(loss/days).toFixed(2),
    weekly:+(loss/days*7).toFixed(2),ratio:+(loss/maxW*100).toFixed(1),
    remaining:+(curW-goal).toFixed(1),bmi,currentStreak:cur,maxStreak:max2,
    startDate:pts[0].date,endDate:pts[pts.length-1].date,etaStr:eta};
}
